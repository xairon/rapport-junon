...
Explicabilité & Attribution multi-méthodes, dont quatre familles accessibles dans l'interface ;
contrefactuels sous contrainte physique avec couplage thermodynamique et validation par modèle
indépendant, livrés en bibliothèque et en API mais non branchés dans l'interface ; détection non
supervisée d'influence anthropique \\
...
physiquement incohérente n'est pas une explication. Ces développements restent préliminaires à
deux titres : ils ne sont pas validés, et la page d'interface qui devait les mettre à portée d'un
hydrogéologue n'est pas branchée dans l'application, de sorte qu'ils ne s'exécutent aujourd'hui
que par l'API ou par un carnet de calcul.
...
Des méthodes d'attribution & Cinq familles complémentaires, confrontables entre elles, dont
quatre accessibles dans l'interface \\
Des explications actionnables & Contrefactuels sous contrainte physique, mettant en œuvre les
critères formulés par les hydrogéologues, appelables par l'API et non par l'interface \\
...
\begin{enumerate}
  ...
  l'évapotranspiration de référence employée par la chaîne climatique. Ce que cet écart change
  pour les calibrations n'a pas été évalué, aucune campagne comparant les deux forçages n'ayant
  été conduite. Les prévisions restent exploitables, mais les paramètres calibrés n'ont pas
  d'interprétation physique directe tant que ce point n'est pas tranché.
  ...
  d'intégration ne comportent que des étapes de construction et de déploiement. Sur la
  ...
  \item \textbf{Les contrefactuels ne sont pas atteignables depuis l'interface.} La page qui les
  pilote existe dans le dépôt, avec son sélecteur de cible, sa vue de comparaison et son appel à
  la validation par modèle indépendant, et tout ce qu'elle appelle fonctionne. Mais elle n'est
  déclarée dans aucune route de l'application, et le laboratoire d'intelligence artificielle
  n'expose que ses quatre onglets de données, d'entraînement, de prévision et de modèles. Les
  méthodes ne s'exécutent donc aujourd'hui que par l'API ou par un carnet de calcul. Le même
  constat vaut, plus modestement, pour les poids d'attention côté attribution. Rebrancher la
  route est court ; ne pas le faire prive de l'outil le public auquel il était destiné.
  ...
  \item \textbf{Le forçage climatique brut inscrit de la géographie dans les modèles
  multivariés.} Le banc d'essai décrit en section~\ref{sec:travaux-perso} montre qu'ajouter les
  covariables climatiques telles quelles à une représentation apprise améliore ses performances
  tout en y rendant la latitude prédictible, deux effets couplés : le gain vient pour l'essentiel
  de la structure géographiquement lisse du forçage, non d'une meilleure lecture du système
  hydrogéologique. Le remède est connu, employer des anomalies par rapport à la moyenne locale de
  longue période ou une recharge dérivée du modèle métier, ce qui conserve l'information
  temporelle du climat en retirant sa composante spatialement lisse. Deux réserves : la mesure a
  été faite sur une grille au quart de degré, quatre fois plus grossière que celle de l'entrepôt,
  et son effet sur la prévision, par opposition à la classification, n'a pas été évalué. Vérifier
  ce point sur la chaîne du chapitre~\ref{ch:entrepot} précède toute interprétation d'un modèle
  multivarié.

  \item \textbf{Deux travaux personnels attendent leur campagne.} Le prototype de
  contrefactuels sous contrainte physique dispose de son cadre, de ses opérateurs, de ses quatre
  sites et de son protocole d'évaluation écrit ; ses tableaux de résultats sont vides, et la
  campagne demande d'abord que le forçage d'évapotranspiration de la chaîne des stations soit
  aligné, faute de quoi la validation par modèle métier hériterait du défaut signalé en première
  ligne de cette liste. Le banc d'essai des représentations, lui, a ses résultats, mais la
  campagne du protocole principal n'est pas versionnée et ses carnets de vérification n'ont
  jamais été exécutés : les rendre reproductibles est un travail court, sans calcul lourd, et
  c'est la condition pour que ces résultats soient opposables à un tiers.
...
numériques. Leur mise à portée effective d'un gestionnaire suppose toutefois le branchement de
l'interface signalé au chapitre~\ref{ch:apprentissage}.
