...
graphique lorsque disponible~\cite{pytorch_lightning}, avec arrêt anticipé et suivi de
...
que sur le réglage. Une recherche automatisée d'hyperparamètres~\cite{optuna} est donc intégrée,
...
\begin{encadre}[title={Une distinction assumée}]
Ces configurations sont des \emph{points de départ raisonnables}, non des réglages optimisés
pour une station donnée. La documentation le dit explicitement, et oriente vers la recherche
automatisée d'hyperparamètres pour aller plus loin. Il aurait été facile — et trompeur — de les
présenter comme optimales.
\end{encadre}
...
campagne d'évaluation comparative — modèles appris contre modèles métier du
...
\begin{limite}[title={Une limite d'ingénierie à corriger}]
...
chaîne d'intégration continue ne comporte qu'une étape de construction d'image, tandis que la
définition de tests existante cible une autre forge. Parmi les quatre échecs, deux exigent un
...
journaux se trouve de fait non testée. C'est le premier correctif à apporter par toute personne
reprenant ces travaux.
\end{limite}

\begin{acquisouvert}
\textbf{Acquis} — Un laboratoire d'expérimentation opérationnel, traçable et accessible, dont le
cœur de calcul est indépendant de l'interface et donc réutilisable.
