...
\begin{table}[H]
\centering
\caption{Inventaire de l'entrepôt de données hydro-climatiques.}
\label{tab:inv-entrepot}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
Période & 23 septembre 2025 au 26 août 2026 \\
Volume & $\approx$ \num{6000} lignes Python, $\approx$ \num{2600} lignes SQL de transformation (mesuré le 26 août 2026, hors tests) \\
Ingestion & Bibliothèque d'extraction déclarative : pagination, reprise sur erreur,
déduplication avant chargement, partitionnement annuel de 1967 à aujourd'hui \\
...
\begin{table}[H]
\centering
\caption{Inventaire de la plateforme d'exploration, de modélisation et de prévision.}
\label{tab:inv-plateforme}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
Période & 6 novembre 2025 au 26 août 2026 \\
Volume & $\approx$ \num{34000} lignes Python, $\approx$ \num{26000} lignes TypeScript (mesuré le 26 août 2026, hors tests) \\
...
de débit \\
Cœur métier & Bibliothèque Python indépendante du service web et de l'interface : préparation, modèles métier,
...
\begin{table}[H]
\centering
\caption{Les quatre jeux de données ingérés et leur couverture.}
\label{tab:sources}
...
depuis 1967 \\
Copernicus ERA5-Land & Réanalyse climatique sur grille \ang{0.1}, France métropolitaine et
Corse & depuis 1950 \\
...
\vspace{0.4em}
Les deux dépôts sont publiés sous licence MIT. Elle couvre le code produit, non les données
consultées : chaque source conserve ses propres conditions d'usage.
\end{encadre}
...
\begin{table}[H]
\centering
\caption{Le cheminement des deux grandeurs d'évapotranspiration, couche par couche.}
\label{tab:etp-couches}
...
Gold, chaîne station & \code{potential\_evaporation} & La variable brute, propagée telle quelle
jusqu'aux chroniques de station. C'est elle qui alimente les modèles métier, et c'est là que se
situe l'incohérence exposée en section~\ref{sec:hargreaves}. \\
\bottomrule
\end{tabularx}
\end{table}
...
plus haut sont issues d'une simple requête sur cette table, et non d'un calcul externe. Elle se
rejoue telle quelle, à condition que la période sur laquelle la mesure a porté, de 2015 à 2025,
soit chargée : la profondeur de la grille climatique est un paramètre d'ingestion, pas une
...
\begin{table}[H]
\centering
\caption{Volumétrie des indicateurs climatiques produits.}
\label{tab:volumetrie-indices}
...
Mailles comparées lors du changement de loi & \num{35614} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Glossaire des termes employés dans ce rapport.}
\label{tab:glossaire}
\begin{tabularx}{\textwidth}{@{}lX@{}}
\toprule
