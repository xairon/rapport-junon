...
La question mérite d'être posée franchement, parce que la réponse par défaut est négative. Un
logiciel de recherche est ordinairement écrit pour produire un résultat, puis abandonné : il
...
Le cœur métier de la plateforme — préparation des données, modèles métier, entraînement,
explicabilité, contrefactuels — ne comporte aucune dépendance à l'interface graphique. Le même
code est appelé depuis l'API web, depuis un script ou depuis un carnet de calcul.

Cette contrainte, tenue depuis l'origine et vérifiée, a un effet direct sur la valeur résiduelle
des travaux : \textbf{même si l'application web était abandonnée, les méthodes resteraient
...
conteneurs — une commande d'arrêt ne peut pas les détruire —, et le chargement initial complet
...
L'honnêteté commande de distinguer ce qui est acquis de ce qui reste à faire. Cinq chantiers
...
\begin{enumerate}
  ...
  à un environnement partagé. Une politique d'accès — qui, pour quels usages, avec quelle
...
usage : ils produisent des scénarios énonçables — « un hiver plus sec et plus chaud d'un degré »
— plutôt que des perturbations numériques.

\begin{acquisouvert}
\textbf{Acquis} — Les propriétés qui distinguent une infrastructure d'un prototype sont
identifiées et effectivement tenues : interface stable, cœur indépendant, indicateurs
centralisés, contrats exécutables, reproductibilité, documentation de reprise. Le patron mis en
œuvre est transposable à d'autres compartiments environnementaux.

\textbf{Ouvert} — Cinq chantiers séparent ce socle d'un environnement d'outils partagé, et la
