...
Elle en donne ce qui est nécessaire à la compréhension du reste du rapport : les propriétés garanties, les décisions qui engagent la validité des résultats, et l'état d'exposition des composants. Le détail complet de la chaîne, source par source et modèle par modèle, avec les procédures d'exploitation et l'inventaire des limites, figure dans le rapport technique qui accompagne ce livrable~\cite{ringuet2026socle} ; il est renvoyé ici plutôt que résumé chaque fois que la précision dépasse ce que la tâche 3.2 requiert.
...
\caption{Le volet nappes et rivières de l'Observatoire, à l'échelle du pays. Chaque pastille agrège les stations d'un secteur et porte la classe de l'indicateur standardisé ; le compteur du bas donne l'assiette réellement classée, distincte du nombre de stations recensées. \lecas}
\label{fig:st32-observatoire}
\end{figure}
...
\begin{figure}[H]
\centering
\vue[0.97]{pastas-ajustement}
\caption{Un modèle mécanistique calibré sur une station. Le niveau observé et le niveau simulé se superposent sur trois décennies ; le trait vertical marque la bascule vers la période réservée, sur laquelle les métriques sont recalculées séparément. C'est ce modèle qui sert de référence indépendante à la validation croisée des contrefactuels. \lecas}
\label{fig:st32-metier}
\end{figure}
...
Un travail conduit en marge de la plateforme invite à la prudence sur ces covariables. Un banc d'essai comparant sept encodeurs de représentation sur \num{5116} chroniques piézométriques, publié à l'adresse \url{https://github.com/xairon/aida_embedding_benchmark}, montre que leur ajout à l'état brut améliore les tâches en aval tout en rendant la latitude prédictible à partir de la représentation, les deux effets progressant de concert : deux stations voisines partagent leur maille climatique, et le forçage inscrit ainsi dans le modèle une structure géographiquement lisse. Le constat a été établi sur une tâche de classification et sur une grille au quart de degré, quatre fois plus grossière que celle de l'entrepôt ; il ne se transporte donc pas tel quel à la prévision. Il importe néanmoins ici, car une attribution d'importance portée sur une covariable climatique brute peut mesurer de la géographie autant que de l'hydrogéologie.
...
\centering
\vue[0.97]{ia-modele}
\caption{Un modèle appris, tel que la plateforme le présente une fois l'entraînement terminé : architecture, station, horizon, fenêtre d'entrée et jeu de test sont attachés au modèle, de sorte qu'une prévision ne puisse être comparée qu'à une autre produite dans les mêmes conditions. \lecas}
\label{fig:st32-apprentissage}
\end{figure}
